...
void PmergeMe::_insertInVector(std::vector<int>& mainChain, std::vector<int>& pendChain)
{
    ...
    {
        int idx = _getJacobsthal(++jacobIdx); // use jacobsthal sequence because it provides good distribution for insertion positions this helps in balancing the tree structure formed during insertion
        ...
        for (size_t i = current_pos + 1; i > limit ; i--)
        {
            ...
            std::vector<int>::iterator it = std::lower_bound(mainChain.begin(), mainChain.end(), val); // we use lower_bound to find the correct position to insert the element to keep the main chain sorted
            mainChain.insert(it, val); // insert the element at the found position
            inserted_count++;
        }
